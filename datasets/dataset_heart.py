...
class RandomGenerator(object):
    ...
    def __str__(self):
        ...
        if self.train:
            return f'Hflip, Vflip, rot90, rotate{self.angle}, cv2_resize'
        else:
            return f'Hflip_{self.Hflip}, Vflip_{self.Vflip}, angle_{self.angle}, cv2_resize'
...
#             image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y, 1), order=2)  # why not 3?
...
class Heart_dataset(Dataset):
    def __init__(self, base_dir, transform=None):
        ...
        self.sample_list = [splitext(file)[0] for file in listdir(self.data_dir) if file.endswith('.png')]
